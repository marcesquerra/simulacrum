...
/**
  ...
  */
@compileTimeOnly(
  "op annotation should have been removed by simulacrum but was not"
)
...
/**
  ...
  */
@compileTimeOnly(
  "noop annotation should have been removed by simulacrum but was not"
)
...
class typeclass(
    ...
    generateAllOps: Boolean = true
) extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any =
    macro TypeClassMacros.generateTypeClass
}
...
class TypeClassMacros(val c: Context) {
  ...
  def generateTypeClass(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    def eliminateVariance(tparam: TypeDef): TypeDef = {
      ...
      val u = c.universe
        .asInstanceOf[c.universe.type with scala.reflect.internal.SymbolTable]
      ...
      TypeDef(
        fixedMods.asInstanceOf[c.universe.Modifiers],
        tparam.name,
        tparam.tparams,
        tparam.rhs
      )
    ...
    def trace(s: => String) = {
      ...
      if (sys.props.get("simulacrum.trace").isDefined)
    ...
    class RewriteTypeName(from: TypeName, to: TypeName) extends Transformer {
      override def transform(t: Tree): Tree = t match {
        ...
        case TypeDef(mods, name, tparams, rhs) if name == from =>
          super.transform(TypeDef(mods, to, tparams, rhs))
    ...
    class FoldTransformer(transformers: List[Transformer]) extends Transformer {
      override def transform(t: Tree): Tree =
        super.transform(
          transformers
            .foldLeft(t)((prev, transformer) => transformer.transform(prev))
        )
    }

    case class Arguments(
        parentsToExclude: Set[TypeName],
        generateAllOps: Boolean
    )

    val typeClassArguments: Arguments = c.prefix.tree match {
      case Apply(_, args) =>
        val excludeParents: Set[TypeName] = args
          .collectFirst {
            case q"excludeParents = $exclusions" =>
              c.eval(c.Expr[List[String]](exclusions))
                .map { n =>
                  TypeName(n)
                }
                .toSet
          ...
          .getOrElse(Set.empty)
        val generateAllOps: Boolean = args
          .collectFirst {
            case q"generateAllOps = $gen" =>
      ...
      case other =>
        c.abort(
          c.enclosingPosition,
          "not possible - macro invoked on type that does not have @typeclass: " + showRaw(
            other
          )
        )
    }
    ...
    def determineOpsMethodName(sourceMethod: DefDef): List[TermName] = {
      ...
      if (suppress) Nil
        ...
        val overrides = sourceMethod.mods.annotations.flatMap { ann =>
          ...
          typed.tpe.typeSymbol.fullName match {
            case "simulacrum.op" =>
              val q"new ${_}(${Literal(Constant(alias: String))}, ..$rest)" =
                typed
              List(genAlias(alias, rest))
            case _ => Nil
          }
        }
        if (overrides.isEmpty) List(sourceMethod.name.toTermName)
        else overrides.flatten
      }
    }
    ...
    def addNotFoundAnnotation(
        mods: Modifiers,
        className: String,
        genericParameter: String
    ): Modifiers = {
      val message =
        s"Could not find an instance of $className for $${$genericParameter}"
      val notFoundAnnotation =
        q"""new _root_.scala.annotation.implicitNotFound($message)"""
      Modifiers(
        mods.flags,
        mods.privateWithin,
        mods.annotations :+ notFoundAnnotation
      )
    }

    def adaptMethodForProperType(
        tcInstanceName: TermName,
        tparamName: Name,
        method: DefDef
    ): List[DefDef] = {
      ...
      for {
        ...
        paramNamess: List[List[Tree]] = {
          val original = method.vparamss map {
            _ map { p =>
              Ident(p.name)
            }
          }
        ...
        rhs = paramNamess.foldLeft(
          Select(Ident(tcInstanceName), method.name): Tree
        ) { (tree, paramNames) =>
    ...
    def adaptMethodForAppliedType(
        tcInstanceName: TermName,
        tparamName: Name,
        method: DefDef,
        liftedTypeArgs: List[TypeDef]
    ): List[DefDef] = {
      val TargetTypeName = tparamName
      (for {
        firstParamList <- method.vparamss.headOption.toList
        ...
        val simpleArgs = {
          def extract(tree: Tree): Option[Name] = tree match {
            ...
            case tq"$ctor[..$targs]" =>
              targs.foldLeft(Option.empty[Name]) { (_, targ) =>
                extract(targ)
              }
          ...
          args.zipWithIndex.map {
            case (arg, idx) =>
              ...
              (
                ...
                simpleArgOpt,
                liftedTypeArgs(idx),
                simpleArgOpt.map(arg equalsStructure Ident(_)).getOrElse(false)
              )
          }
        }
        ...
        if (skipMethod) List.empty
        else {
          //rewrites all occurrences of any of the args which are defined on the method to the lifted arg
          val rewriteSimpleArgs = new FoldTransformer(
            simpleArgs.foldLeft(List.empty[Transformer]) {
              case (ts, (_, Some(simpleArg), liftedTypeArg, _)) =>
                new RewriteTypeName(
                  from = simpleArg.toTypeName,
                  to = liftedTypeArg.name
                ) :: ts
            }
          )
          ...
          val withRewrittenFirst = withoutFirst map {
            _ map { param =>
              ValDef(
                param.mods,
                param.name,
                rewriteSimpleArgs.transform(param.tpt),
                rewriteSimpleArgs.transform(param.rhs)
              )
            }
          }

          val paramssFixed =
            if (equalityEvidences.isEmpty) withRewrittenFirst
            else {
              if (withRewrittenFirst.nonEmpty && withRewrittenFirst.last.head.mods
                    .hasFlag(Flag.IMPLICIT))
                withRewrittenFirst.init ++ List(
                  equalityEvidences ++ withRewrittenFirst.last
                )
          ...
          val paramNamess: List[List[Tree]] = {
            ...
            val replacement =
              if (equalityEvidences.isEmpty) q"self"
              else q"self.asInstanceOf[${tparamName.toTypeName}[..$args]]"
            original.updated(0, original(0).updated(0, replacement))
          }

          val mtparamss =
            if (equalityEvidences.isEmpty)
              method.tparams
                .map(t => tq"""${t.name}""")
                .map(rewriteSimpleArgs.transform)
            else Nil

          val rhs = paramNamess.foldLeft(
            q"""$tcInstanceName.${method.name.toTermName}[..$mtparamss]""": Tree
          ) { (tree, paramNames) =>
            Apply(tree, paramNames)
          }

          val fixedTParams = method.tparams.filter { tparam =>
            !removeTParams.contains(tparam.name)
          }
    ...
    def adaptMethods(
      ...
      def matchMethod: PartialFunction[Tree, DefDef] = {
        case m: DefDef
            if !m.mods.hasFlag(Flag.PRIVATE) && !m.mods.hasFlag(
              Flag.PROTECTED
            ) =>
          m
        case t if t.isInstanceOf[Trees#DocDef] =>
          matchMethod(
      ...
      typeClassMethods.flatMap { method =>
        val adapted =
          if (proper)
            adaptMethodForProperType(tcInstanceName, tparamName, method)
          else
            adaptMethodForAppliedType(
              tcInstanceName,
              tparamName,
              method,
              liftedTypeArgs
    ...
    def targetTypeTree(
      ...
      if (proper) tq"${tparam.name}"
      else tq"""${tparam.name}[..${liftedTypeArgs.map(_.name)}]"""

    def refinedInstanceTypeTree(
        typeClass: ClassDef,
        tparam: TypeDef,
        instance: TermName
    ): Tree = {
      val abstractTypeMembers = typeClass.impl.children.collect {
        case t @ TypeDef(mods, _, _, _) if mods.hasFlag(Flag.DEFERRED) => t
      }
      if (abstractTypeMembers.isEmpty) {
        tq"${typeClass.name}[${tparam.name}]"
      } else {
        val refinements = abstractTypeMembers.map {
          case TypeDef(mods, name, tparams, rhs) =>
            val (namedParams, names) = tparams.map {
              case TypeDef(pmods, _, ptparams, prhs) =>
    ...
    def generateOps(
        ...
        proper: Boolean,
        liftedTypeArgs: List[TypeDef]
    ): (ClassDef, Set[TypeName]) = {
      val adaptedMethods = adaptMethods(
        typeClass,
        tcInstanceName,
        tparam.name,
        proper,
      ...
      val shouldImportTcMembers = {
        val typeMembersOfTypeClass = typeClass.impl.children.collect {
          case t: TypeDef => t
        }
      ...
      val importTcMembers =
        if (shouldImportTcMembers) List(q"""import $tcInstanceName._""")
        else Nil
      ...
      val reservedTypeNames = adaptedMethods
        .flatMap(_.tparams.map(_.name))
        .toSet ++ tparamNames
    ...
    def generateAllOps(
      ...
      val typeClassParents: List[TypeName] = typeClass.impl.parents.collect {
        case tq"${Ident(parentTypeClassTypeName)}[${_}]" =>
          parentTypeClassTypeName.toTypeName
      }
      ...
      val unknownParentExclusions =
        (typeClassArguments.parentsToExclude diff typeClassParents.toSet).toList
          .map(_.toString)
          .sorted
      if (unknownParentExclusions.nonEmpty) {
        c.error(
          c.enclosingPosition,
          s"@typeclass excludes unknown parent types: ${unknownParentExclusions.mkString}"
        )
      }
    ...
    def generateCompanion(
      ...
      val summoner =
        q"@scala.inline def apply[$tparam](implicit $instance: ${typeClass.name}[${tparam.name}]): $refinedType = $instance"

      val liftedTypeArgs =
        if (proper) List.empty[TypeDef]
        else {
          ...
          if (ftss.isEmpty)
            c.abort(
              c.enclosingPosition,
              "Cannot find a wildcard type in supposed n-arity type constructor"
            )
          else {
            val liftedTypeArgName = TypeName(c.freshName(s"lta"))
            ftss
              .foldLeft(0 -> List.empty[TypeDef]) {
                case ((i, ts), q"$mods type ${_}[..$tpps] = $rhs") =>
                  val fixedMods =
                    Modifiers(NoFlags, mods.privateWithin, mods.annotations)
                  val tname = TypeName(c.freshName(s"lta$i"))
                  object rewriteWildcard extends Transformer {
                    override def transform(t: Tree): Tree = t match {
                      case Ident(typeNames.WILDCARD) =>
                        super.transform(Ident(tname))
                      case _ => super.transform(t)
                    }
                  }
                  (i + 1) -> (rewriteWildcard
                    .transformTypeDefs(
                      List(TypeDef(fixedMods, tname, tpps, rhs))
                    )
                    .head :: ts)
              }
              ._2
      ...
      val (opsTrait, opsReservedTParamNames) =
        generateOps(typeClass, tcInstanceName, tparam, proper, liftedTypeArgs)
      val allOpsTrait =
        generateAllOps(typeClass, tcInstanceName, tparam, liftedTypeArgs)

      def generateOpsImplicitConversion(
          opsType: TypeName,
          methodName: TermName
      ) = {
        val tparams = List(eliminateVariance(tparam)) ++ liftedTypeArgs
      ...
      val toOpsTrait = {
        ...
        val method = generateOpsImplicitConversion(
          opsTrait.name,
          TermName(s"to${typeClass.name}Ops")
        )
      ...
      val allOpsConversion = {
        val method = generateOpsImplicitConversion(
          TypeName("AllOps"),
          TermName(s"toAll${typeClass.name}Ops")
        )
        q"object ops { $method }"
      }
      ...
      val opsMembers: List[Tree] = {
        ...
        val allOps =
          if (typeClassArguments.generateAllOps)
            List(allOpsTrait, allOpsConversion)
          else Nil
        ops ++ allOps
      }
      ...
      liftedTypeArgs
        .foldLeft((companion: Tree) -> potentialNames) {
          case ((prev, namesLeft), lta) =>
            val newName = namesLeft.head
            new RewriteTypeName(from = lta.name, to = newName)
              .transform(prev) -> namesLeft.tail
        }
        ._1
    }
    ...
    def modify(typeClass: ClassDef, companion: Option[ModuleDef]) = {
      ...
      val modifiedTypeClass = {
        ...
        val modifiedParents = {
          ...
          val universal =
            if (makeUniversal) List(tq"_root_.scala.Any")
            else typeClass.impl.parents
          universal :+ tq"_root_.scala.Serializable"
        }
        val filteredImpl =
          Template(modifiedParents, typeClass.impl.self, filteredBody)
      ...
      val modifiedCompanion =
        generateCompanion(typeClass, tparam, proper, companion match {
          case Some(c) => c
      ...
      """)
      trace(
        s"Generated type class ${typeClass.name}:\n" + showCode(result.tree)
      )
    ...
    annottees.map(_.tree) match {
      ...
      case (typeClass: ClassDef) :: (companion: ModuleDef) :: Nil =>
        modify(typeClass, Some(companion))
      case other :: Nil =>
        c.abort(
          c.enclosingPosition,
